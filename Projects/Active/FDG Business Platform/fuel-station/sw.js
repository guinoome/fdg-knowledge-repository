...
self.addEventListener("activate", (event) => {
  event.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((key) => key.startsWith("fdg-fuel-station-") && key !== CACHE).map((key) => caches.delete(key)))).then(() => self.clients.claim()));
});
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(caches.open(CACHE).then((cache) => cache.match(event.request)).then((cached) => cached ?? fetch(event.request).then((response) => {
    ...
    return response;
  }).catch(() => event.request.mode === "navigate" ? caches.open(CACHE).then((cache) => cache.match("./index.html")) : Response.error())));
});
