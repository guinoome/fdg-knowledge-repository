...
function MetaRow({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-black/12 py-3 last:border-0">
      <dt className="flex shrink-0 items-center gap-2 text-xs text-black/48">
...
function Compatibility({
  ...
  return (
    <span className={cn("inline-flex items-center gap-2 text-xs", !supported && "text-black/42")}>
      {supported ? (
  ...
  return (
    <>
      <section className="relative -mx-4 -mt-8 min-h-[72svh] overflow-hidden bg-black text-white md:-mx-8">
        ...
        <div className="relative z-10 mx-auto flex min-h-[72svh] max-w-7xl flex-col justify-between px-6 py-8 md:px-10 md:py-12">
          <Link
            ...
            className="inline-flex w-fit items-center gap-2 text-[9px] font-semibold uppercase tracking-[.24em] text-white/62 transition hover:text-white"
          >
          ...
          <div className="max-w-3xl pb-8">
            <p className="text-[9px] font-semibold uppercase tracking-[.28em] text-[#dfbd7e]">
              {proof?.occasion ?? template.category.name} · {proof?.tier ?? template.tier}
            </p>
      ...
      {searchParams.selected === "1" ? (
        <div role="status" className="mt-8 flex items-start gap-3 border border-[#8b6735]/30 bg-[#8b6735]/5 p-4 text-sm">
          <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-[#8b6735]" aria-hidden="true" />
          <div>
            <p className="font-semibold">Experience selected</p>
            <p className="mt-1 text-black/55">Your choice is saved and ready for the guided invitation builder.</p>
          </div>
        </div>
      ) : null}
      ...
      {proof ? (
        <section className="py-20 md:py-28">
          <div className="text-center">
            <p className="text-[9px] font-semibold uppercase tracking-[.28em] text-[#8b6735]">The guest journey</p>
          ...
          <ol className="mt-14 grid gap-px overflow-hidden bg-black/12 sm:grid-cols-2 lg:grid-cols-7">
            {JOURNEY.map(({ label, icon: Icon }, index) => (
              <li key={label} className="bg-[#f4f0e8] px-5 py-8 text-center">
                <Icon className="mx-auto size-5 text-[#8b6735]" aria-hidden="true" />
                <p className="mt-5 text-[9px] font-semibold uppercase tracking-[.22em]">0{index + 1} · {label}</p>
              </li>
            ))}
          </ol>
      ...
      <section className="grid gap-12 border-t border-black/15 py-20 md:grid-cols-[1.25fr_.75fr] md:py-28">
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-[.28em] text-[#8b6735]">About this experience</p>
          <h2 className="mt-5 max-w-2xl font-serif text-5xl leading-[.96] tracking-[-.04em]">Made to become your story.</h2>
          <p className="mt-7 max-w-2xl text-sm leading-7 text-black/58">{template.description}</p>
          {template.tags.length > 0 ? (
            <ul className="mt-8 flex flex-wrap gap-x-5 gap-y-2 text-[9px] font-semibold uppercase tracking-[.2em] text-black/48">
              {template.tags.map((tag) => <li key={tag}>{facetLabel(tag)}</li>)}
            </ul>
          ) : null}
          <p className="mt-8 max-w-xl text-sm leading-7 text-black/58">
            Your cover and gallery photographs replace the sample media. The experience choreography, responsive layout, accessibility, and interaction design stay carefully composed.
          </p>
        </div>
        ...
        <aside className="border-t border-black/15 pt-6 md:border-l md:border-t-0 md:pl-10 md:pt-0">
          ...
          <dl className="mt-5">
            <MetaRow icon={User} label="Designer">{template.designer}</MetaRow>
            <MetaRow icon={Tag} label="Category">{template.category.name}</MetaRow>
            <MetaRow icon={Tag} label="Version"><span className="font-mono">{template.version}</span></MetaRow>
            <MetaRow icon={CalendarDays} label="Updated">
              <time dateTime={template.updatedAt.toISOString()}>
                {template.updatedAt.toLocaleDateString("en-PH", { year: "numeric", month: "short", day: "numeric" })}
              </time>
            </MetaRow>
          </dl>
          <div className="mt-7 flex flex-col gap-3 border-t border-black/15 pt-6">
            <Compatibility supported={template.websiteCompatible} label="Interactive event website" />
            <Compatibility supported={template.printCompatible} label="Matching printed invitation" />
          </div>
          {template.features.length > 0 ? (
            <ul className="mt-7 flex flex-wrap gap-2">
              {template.features.map((feature) => (
                <li key={feature} className="border border-black/15 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[.16em] text-black/52">
                  {facetLabel(feature)}
                </li>
              ))}
            </ul>
          ...
          {collections.length > 0 ? (
            <p className="mt-7 text-xs text-black/48">Part of the {collections.map((collection) => collection.name).join(", ")} collection{collections.length > 1 ? "s" : ""}.</p>
          ) : null}
        </aside>
      </section>
    </>
