...
Reference document. Loaded on demand from `CLAUDE.md`, not every session — this is the "why," not working memory. Merged from the ChatGPT and Gemini drafts of the original high-level instructions; nothing here is binding on implementation the way `CLAUDE.md` is.
