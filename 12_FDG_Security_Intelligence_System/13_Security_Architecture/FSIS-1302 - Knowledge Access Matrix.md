# FSIS-1302 — Knowledge Access Matrix

Baseline access model:

| Asset | Public | Internal | Confidential | Restricted | FDG CORE |
|---|---:|---:|---:|---:|---:|
| Public documentation | R | R | R | R | R |
| General standards | - | R | R | R | R |
| Domain implementation | - | R | R | Controlled | Controlled |
| Business strategy | - | - | R | Controlled | Controlled |
| Security architecture | - | - | - | R | R |
| Master FDG architecture | - | - | - | Controlled | R |
| Owner strategy | - | - | - | - | R |
| Credentials/secrets | - | - | - | - | Controlled |

This is a governance baseline. Technical implementations must map the model to actual capabilities.
